...
int start_remote_control(int argc, char* argv[]) {
	...
	for (k=0; k < argc; k++) {
		char *p = argv[k];
		if(k > 0)
			rfbLog("start_remote_control >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> %d = %s\n", k, p);
	}
...
void* start_main(void* ptr){
	...
	setStatusNotify(STATUS_NOTIFY_BEGIN);

	rfbLog("start_main >>>>>>>>>>> server[%s] acceptNum[%s]\n", waitInfo->webServerHost, waitInfo->acceptNum);

	waitInfo->success = false;

	rfbLog("scheme = %s\n", waitInfo->webServerScheme);

	setStatusNotify(STATUS_NOTIFY_REQ_ACCEPTNUM);

	bool success;
	success = request_acceptnum(waitInfo);

	if(!success){
		ui_finish("[Failed] request valid start remote control.");
		return false;
	}
	else if(waitInfo->reqResult == REQ_RESULT_ACCEPTNUM_EMPTY){
		ui_resume("[Failed] agent is not wait number");
		return false;
	}
	...
	sprintf(connectString, "%s:%d", waitInfo->relayHost, waitInfo->relayPort);

	rfbLog("connect string >> %s\n", connectString);
	...
	start_remote_control(argc, argv);
}
