...
//yh
#include "x11vnc/w_common.h"
#include "x11vnc/w_ui_overlay.h"
                     ...
                     rfbBool isUDP)
{
	...
	for (i = 0; i < 4; i++)
	  ...
	  //yh
      bool success = start_relay(cl);
      if(!success){
    	  return NULL;
      }


	  cl->rfbDataEncryptEnable = FALSE;
      rfbDataEncryptCheckMsg cs;
      cs.type = rfbDataEncryptCheck;
      cs.DataEncrypt = TRUE;
      if (rfbWriteExact(cl, ((char *)&cs), sz_rfbDataEncryptCheckMsg) < 0) {
    	  rfbLogPerror(">>>>> rfbDataEncryptCheckMsg: write");
    	  rfbCloseClient(cl);
    	  rfbClientConnectionGone(cl);
    	  return NULL;
      }
      rfbLog("write encrypt enable\n");

      rfbDataEncryptCheckMsg cr;
	  if (rfbReadExact(cl, ((char *)&cr), sz_rfbDataEncryptCheckMsg) < 0) {
		  rfbLogPerror(">>>>> rfbDataEncryptCheckMsg: read");
		  rfbCloseClient(cl);
		  rfbClientConnectionGone(cl);
		  return NULL;
	  }
	  cl->rfbDataEncryptEnable = cr.DataEncrypt;
	  rfbLog("read encrypt enable >> %d\n", cl->rfbDataEncryptEnable);
...
rfbProcessClientNormalMessage(rfbClientPtr cl)
{
    ...
    case rfbPenMode:
    	...
    	if ((n = rfbReadExact(cl, ((char *)&msg) + 1, sz_rfbPenModeMsg - 1)) <= 0) {
    		if (n != 0)
    			rfbLogPerror("rfbProcessClientNormalMessage: read");
			rfbCloseClient(cl);
			return;
    	...
    	RedpenInfo penInfo;
		penInfo.onoff 	= msg.rpm.onoff;
		penInfo.width 	= Swap16IfLE(msg.rpm.width);
		penInfo.color 	= Swap32IfLE(msg.rpm.color);
		penInfo.PenType = Swap16IfLE(msg.rpm.PenType);
		penInfo.erase 	= Swap16IfLE(msg.rpm.erase);

    	rfbLog("rPenModeMsg => on:%d width:%d color:%d penType:%d\n",
    			penInfo.onoff, penInfo.width, penInfo.color, penInfo.PenType);


    	isPenMode = (msg.rpm.onoff==1?true:false);

    	if(isPenMode)
    		overlay_create(penInfo);
    	else
    		overlay_destroy();

    	//sendPenModeMessage(msg.rpm.onoff, Swap16IfLE(msg.rpm.PenType),
    	//		Swap16IfLE(msg.rpm.width), Swap32IfLE(msg.rpm.color), Swap16IfLE(msg.rpm.erase));
    ...
    case rfbPointerEvent:
		...
		if(!cl->viewOnly) {
			...
			int x = Swap16IfLE(msg.pe.x);
			int y = Swap16IfLE(msg.pe.y);
			...
			if (msg.pe.buttonMask != cl->lastPtrButtons ||
				...
				//yh
				if(isPenMode){
					if(lastButtonMask == 0 && msg.pe.buttonMask == 1){

						PointerEvent evt;
						evt.action = POINTER_ACTION_DOWN;
						evt.x = ScaleX(cl->scaledScreen, cl->screen, x);
						evt.y = ScaleY(cl->scaledScreen, cl->screen, y);
						evt.mask = 0;
						overlay_pointer_event_message(evt);

						lastButtonMask = 1;
					}
					else if(lastButtonMask == 1 && msg.pe.buttonMask == 0){

						PointerEvent evt;
						evt.action = POINTER_ACTION_UP;
						evt.x = ScaleX(cl->scaledScreen, cl->screen, x);
						evt.y = ScaleY(cl->scaledScreen, cl->screen, y);
						evt.mask = 0;
						overlay_pointer_event_message(evt);

						lastButtonMask = POINTER_ACTION_DOWN;
					}
					else if(lastButtonMask == 1 && msg.pe.buttonMask == 1){
						PointerEvent evt;
						evt.action = POINTER_ACTION_MOVE;
						evt.x = ScaleX(cl->scaledScreen, cl->screen, x);
						evt.y = ScaleY(cl->scaledScreen, cl->screen, y);
						evt.mask = 0;
						overlay_pointer_event_message(evt);
					...
					cl->screen->ptrAddEvent(msg.pe.buttonMask,
							ScaleX(cl->scaledScreen, cl->screen, x),
							ScaleY(cl->scaledScreen, cl->screen, y),
							cl);
				...
				if(!isPenMode){
					cl->lastPtrX = ScaleX(cl->scaledScreen, cl->screen, x);
					cl->lastPtrY = ScaleY(cl->scaledScreen, cl->screen, y);
					cl->lastPtrButtons = msg.pe.buttonMask;
				}
			}
		}
